...
def log_b(x, b):
    ...
    denominator = tf.log(tf.constant(b, dtype=numerator.dtype) + eps)
...
class FrankTNorm(TNorm):
    ...
    def __call__(self, args):
        ...
        if self.p == 0:
            ...
            return log_b(1. + (tf.pow(p, x) - 1) * (tf.pow(p, y) - 1) / (p - 1), p)
...
class FrankGenerator(Generator):
    ...
    def __call__(self, x):
        ...
        res = tf.case(
            pred_fn_pairs = [(tf.equal(p,1.), lambda:- tf.log(x +  1e-6)),
                             (tf.equal(p,float('inf')), lambda: 1 - x)],
            default= lambda: tf.log((p - 1.)/(tf.pow(p, r(x)) - 1)),
            exclusive = True)
        return res
...
class SSTNorm(TNorm):
    ...
    def __init__(self, p=None):
        ...
        self.p = p if p == "inf" or p =="-inf" else 1. * p
    ...
    def __call__(self, args):
        ...
        if self.p == "-inf":
            ...
            return tf.pow(tf.maximum(0., tf.pow(r(x), p) + tf.pow(r(y), p) - 1), 1/p)
        elif self.p == "inf":
            return tf.maximum(0., x + y - 1.)
...
class SSGenerator(Generator):
    ...
    def __call__(self, x):
        p = self.p
        return tf.cond(tf.equal(p,0),
                lambda: - tf.log(x + eps),
                lambda: (1 - tf.pow(x+eps, p)) / p)
...
class ForAllT(FormulaTensor, Quantifier):
    ...
    def apply_op(self,args):
        print("LOGIC Evaluation do not support Quantifiers. Weak Quantifiers are exploited")
...
def forall(variable, formula):
    res = LogicFactory.create(FORALL)(variable, formula)
    return res
