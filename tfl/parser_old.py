...
class Term(PNode):
    def __init__(self, world):
        super(Term, self).__init__(None, world)
...
class Number(Term):
    ...
    def __init__(self, tokens, constraint, world):
        super(Number, self).__init__(world)
...
class Constant(Term):
    ...
    """
    def __init__(self, name, constraint, world, domain=None, index=None):
        super(Constant, self).__init__(world)
        self.name = name
        self.constraint = constraint
        if domain is None:
...
class Variable(Term):
    ...
    def __init__(self, name, constraint, world):
        super(Variable, self).__init__(world)
        self.label = name
        self.constraint = constraint
...
def create_or_get_variable(tokens, constraint, world):
    var_name = tokens[0]
    if var_name in constraint.variables_dict:
        return constraint.variables_dict[var_name]
    else:
        new_var = Variable(var_name, constraint, world)
...
class Function(Term):
    cache = {}
    def __init__(self, t, constraint, world):
        super(Function, self).__init__(world)
        self.constraint = constraint
        self.label = t[0]
...
class Atomic(PNode):
    cache = {}
    def __init__(self, t, constraint, world):
        super(Atomic, self).__init__(t, world)
        self.constraint = constraint
        self.label = t[0]
...
class Op(PNode):
    ...
    def __init__(self, t, world):
        super(Op, self).__init__(t, world)
        self.args = t[0][0::2]
        self.label = t[0][1]
...
class And(Op):
    ...
    def __init__(self, t, world):
        super(And, self).__init__(t, world)

    def compile(self):
        super(And, self).compile()
        with tf.name_scope("And"):
            self.tensor = self.world.logic.weak_conj([a.tensor for a in self.args])
...
class Or(Op):
    ...
    def __init__(self, t, world):
        super(Or, self).__init__(t, world)

    def compile(self):
        super(Or, self).compile()
        with tf.name_scope("Or"):

            self.tensor = self.world.logic.strong_disj([a.tensor for a in self.args])

class Xor(Op):


    def __init__(self, t,world):
        super(Xor, self).__init__(t,world)

    def compile(self):
        super(Xor, self).compile()
        with tf.name_scope("Xor"):

            self.tensor = self.world.logic.exclusive_disj([a.tensor for a in self.args])


class Iff(Op):
    def __init__(self, t,world):
        super(Iff, self).__init__(t, world)
    ...
    def compile(self):
        ...
        with tf.name_scope("Iff"):
            self.tensor = self.world.logic.iff(self.args[0].tensor, self.args[1].tensor)
...
class Implies(Op):
    ...
    def __init__(self, t, world):
        super(Implies, self).__init__(t, world)
    ...
    def compile(self):
        ...
        with tf.name_scope("Implies"):
            self.tensor = self.world.logic.implication(self.args[0].tensor, self.args[1].tensor)
...
class Not(PNode):
    ...
    def __init__(self, t, world):
        super(Not, self).__init__(t,world)
        self.args = [t[0][1]]
        self.label = 'not'
    ...
    def compile(self):
        ...
        with tf.name_scope("Not"):
            self.tensor = self.world.logic.negation(self.args[0].tensor)
...
class Quantifier(PNode):
    ...
    def __init__(self, t,world):
        super(Quantifier, self).__init__(t,world)
        self.args = [t[2][0]]
        self.label = t[0]
        self.var = t[1]
...
class ForAll(Quantifier):
    def __init__(self, constraint, t, world):
        super(ForAll, self).__init__(t, world)
        self.constraint = constraint
    ...
    def compile(self):
        ...
        with tf.name_scope("ForAll"):
            ...
            self.tensor = self.world.logic.forall(self.args[0].tensor, var_axis)
            self.constraint.cartesian_shape[var_axis] = 1


class Exists(Quantifier):
    def __init__(self, constraint, t, world):
        super(Exists, self).__init__(t, world)
        self.constraint = constraint
    ...
    def compile(self):
        ...
        with tf.name_scope("Exists"):
            var_axis = self.constraint.variable_indices[self.var.label]
            self.tensor = self.world.logic.exists(self.args[0].tensor, var_axis)
            self.constraint.cartesian_shape[var_axis] = 1


class Exists_n(Quantifier):
    def __init__(self, constraint, t, world):
        super(Exists_n, self).__init__(t, world)
        self.constraint = constraint
        self.args = [t[3][0]]
        self.label = t[0]
        self.n = int(t[1])
    ...
    def compile(self):
        ...
        with tf.name_scope("Exists_n"):
            var_axis = self.constraint.variable_indices[self.var.label]
            self.tensor = self.world.logic.exists_n(self.args[0].tensor, var_axis, self.n)
            self.constraint.cartesian_shape[var_axis] = 1
...
class FOLParser(object):
    ...
    def _createParseAction(self, class_name, constraint):
        def _create(tokens):
            if class_name == "Number":
                return Number(tokens, constraint, self.world)
            elif class_name == "Variable":
                return create_or_get_variable(tokens, constraint, self.world)
            elif class_name == "Constant":
                return create_or_get_constant(tokens, constraint, self.world)
            elif class_name == "Function":
                return Function(tokens, constraint, self.world)
            ...
            #     return Divide(tokens, constraint)
                ...
                return Atomic(tokens, constraint, self.world)
            elif class_name == "NOT":
                return Not(tokens, self.world)
            elif class_name == "AND":
                return And(tokens, self.world)
            elif class_name == "OR":
                return Or(tokens, self.world)
            elif class_name == "XOR":
                return Xor(tokens, self.world)
            elif class_name == "IMPLIES":
                return Implies(tokens, self.world)
            elif class_name == "IFF":
                return Iff(tokens, self.world)
            elif class_name == "FORALL":
                return ForAll(constraint, tokens, self.world)
    ...
    def parse(self, definition, constraint):
        ...
        const = oneOf(list(self.world.individuals.keys())) | oneOf(list(self.world.domains.keys()))+ left_square +Word(nums) + right_square
        const.setParseAction(self._createParseAction("Constant", constraint))

        func = oneOf(list(self.world.functions.keys()))
        ...
        relation = oneOf(list(self.world.relations.keys()))
        ...
        domain = oneOf(self.world.domains.keys())
