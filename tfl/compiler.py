...
from .utils import IDProvider, cartesian
import abc
import numpy as np
...
class LogicTensor(tf.Tensor):
    ...
    def __init__(self, tensor, name=None):
        ...
        super(LogicTensor, self).__init__(op=tensor.op,
...
class VariableDependentTensor(LogicTensor, ):
...
class AtomTensor(FormulaTensor):
    ...
    def __init__(self, predicate, variables, tensor=None):
        ...
        for i, v in enumerate(variables):
            assert v.domain==predicate.domains[i] or predicate.domains[i] in v.domain.ancestors
    ...
    if World._evaluation_mode==LOSS_MODE:
        ...
        for i, a in enumerate(args):
            if isinstance(a,AtomTensor):
                args[i] = VariableDependentTensor(tensor=World.generator(a), variables=a.variables_list)
        return tuple(args)
    else:
        return args
    ...
    for i,v in enumerate(sorted_vars):
        for j, arg in enumerate(args):
            if v not in arg.variables_list:
                ...
                args[j] = VariableDependentTensor(temp, tuple(set(args[j].variables_list).union([v])))
    return tuple(args)
